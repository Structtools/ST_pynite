...
_PRESETS = {
    1: dict(
        ...
        H=3.0, B1=5.0, B2=5.0,
        COLUMN_SEC='IPE200', BEAM_SEC='IPE200',
        _base='FIXED',
        BEAM_LOAD_1=-10e3, BEAM_LOAD_2=-10e3,
        LOAD_B=0.0, LOAD_C=0.0, LOAD_D=0.0,
        LATERAL_BRACE_BEAM_NODES=False,
        N_ELEM=8, NUM_MODES=5,
    ),
    2: dict(
        name='2-bay, fixed bases, 10 kN/m UDL + DZ bracing  [PolyFrame in-plane alpha_cr]',
        H=3.0, B1=5.0, B2=5.0,
        COLUMN_SEC='IPE200', BEAM_SEC='IPE200',
        _base='FIXED',
        BEAM_LOAD_1=-10e3, BEAM_LOAD_2=-10e3,
        LOAD_B=0.0, LOAD_C=0.0, LOAD_D=0.0,
        LATERAL_BRACE_BEAM_NODES=True,
        N_ELEM=8, NUM_MODES=5,
    ),
    3: dict(
        name='1-bay, fixed bases, 10 kN/m UDL',
        H=3.0, B1=5.0, B2=0.0,
        COLUMN_SEC='IPE200', BEAM_SEC='IPE200',
        _base='FIXED',
        BEAM_LOAD_1=-10e3, BEAM_LOAD_2=0.0,
        LOAD_B=0.0, LOAD_C=0.0, LOAD_D=0.0,
        LATERAL_BRACE_BEAM_NODES=False,
        N_ELEM=8, NUM_MODES=5,
    ),
    4: dict(
        name='1-bay, pinned bases, 10 kN/m UDL',
        H=3.0, B1=5.0, B2=0.0,
        COLUMN_SEC='IPE200', BEAM_SEC='IPE200',
        _base='PINNED',
        BEAM_LOAD_1=-10e3, BEAM_LOAD_2=0.0,
    ...
    5: dict(
        name='2-bay, fixed bases, 100 kN column-top point loads',
        H=3.0, B1=5.0, B2=5.0,
        COLUMN_SEC='IPE200', BEAM_SEC='IPE200',
        _base='FIXED',
        BEAM_LOAD_1=0.0, BEAM_LOAD_2=0.0,
        LOAD_B=-100e3, LOAD_C=-100e3, LOAD_D=-100e3,
        LATERAL_BRACE_BEAM_NODES=False,
        N_ELEM=8, NUM_MODES=5,
    ),
    6: dict(
        name='2-bay, pinned bases, 100 kN column-top point loads',
        H=3.0, B1=5.0, B2=5.0,
        COLUMN_SEC='IPE200', BEAM_SEC='IPE200',
        _base='PINNED',
        BEAM_LOAD_1=0.0, BEAM_LOAD_2=0.0,
    ...
    7: dict(
        name='2-bay, fixed bases, H=5 m, 10 kN/m UDL  [taller frame]',
        H=5.0, B1=5.0, B2=5.0,
        COLUMN_SEC='IPE200', BEAM_SEC='IPE200',
        _base='FIXED',
        BEAM_LOAD_1=-10e3, BEAM_LOAD_2=-10e3,
    ...
    8: dict(
        name='2-bay, fixed bases, IPE300 cols / IPE200 beams, 20 kN/m UDL',
        H=3.0, B1=5.0, B2=5.0,
        COLUMN_SEC='IPE300', BEAM_SEC='IPE200',
        _base='FIXED',
        BEAM_LOAD_1=-20e3, BEAM_LOAD_2=-20e3,
        LOAD_B=0.0, LOAD_C=0.0, LOAD_D=0.0,
        LATERAL_BRACE_BEAM_NODES=False,
        N_ELEM=8, NUM_MODES=5,
    ),
...
# ============================================================================
# COLUMN PRESETS  (scenarios 9-12) — isolated single-column models
# BCs: (DX, DY, DZ, RX, RY, RZ) in global frame.
# Column runs along global Y → torsion = RY, bending in XY plane = RZ.
# ============================================================================
_COLUMN_PRESETS = {
    9: dict(
        name='Isolated column - pinned-pinned  (K = 1.0)',
        H=3.0, section='IPE200', P=-100e3, N_ELEM=10,
        K_theory=1.0,
        bc_base=(True,  True,  True,  False, True,  False),
        bc_top =(True,  False, True,  False, True,  False),
    ),
    10: dict(
        # H=6m so L_cr=0.5*6=3.0m = same effective length as pin-pin/H=3m.
        # With IPE200, N_cr,flex(fix-fix,H=6)=311kN < N_cr,torsion=765kN,
        # so the first eigenmode is flexural (not torsional).
        name='Isolated column - fixed-fixed, no sway  (K = 0.5, H = 6 m)',
        H=6.0, section='IPE200', P=-100e3, N_ELEM=10,
        K_theory=0.5,
        bc_base=(True,  True,  True,  True,  True,  True),
        bc_top =(True,  False, True,  True,  True,  True),
    ),
    11: dict(
        name='Isolated column - fixed-free cantilever  (K = 2.0)',
        H=3.0, section='IPE200', P=-100e3, N_ELEM=10,
        K_theory=2.0,
        bc_base=(True,  True,  True,  True,  True,  True),
        bc_top =(False, False, False, False, False, False),
    ),
    12: dict(
        name='Isolated column - fixed-pinned, no sway  (K = 0.699)',
        H=3.0, section='IPE200', P=-100e3, N_ELEM=10,
        K_theory=0.699,
        bc_base=(True,  True,  True,  True,  True,  True),
        bc_top =(True,  False, True,  False, True,  False),
...
# ============================================================================
# BEAM M_CR PRESET  (scenario 13) — Annex F formula, step-by-step
# PyNite's 6-DOF elements cannot compute LTB via eigenvalue (no warping DOF).
# M_cr is therefore computed analytically from the Annex F / NCCI SN003
# formula.  The scenario shows each intermediate term for hand verification.
# ============================================================================
_BEAM_MCR_PRESETS = {
    13: dict(
        name='Simply supported beam - M_cr via EN 1993-1-1 Annex F',
        L=5.0, section='IPE200',
        w=-10e3,      # N/m UDL (downward)
        C1=1.132,     # UDL on simply supported beam (from tables)
        k=1.0,        # lateral rotation restraint factor (both ends free)
        kw=1.0,       # warping restraint factor (both ends free to warp)
        Iw=12900e-12, # m^6 — warping constant for IPE200 (12 900 cm^6)
        N_ELEM=10,
    ),
}
...
def _apply_scenario():
    ...
    if SCENARIO == 0:
        return
    if SCENARIO in _COLUMN_PRESETS or SCENARIO in _BEAM_MCR_PRESETS:
        return   # handled by their own runners
...
def _add_column(model, name, base_node, top_node, mat, sec,
                n_elem, col_height, base_x, base_support):
...
def _add_beam(model, name, left_node, right_node, mat, sec,
              n_elem, y_level, x_left, x_right):
...
def build_model():
    ...
    # Columns
    _add_column(model, 'Col1', 'A', 'B', 'Steel', COLUMN_SEC,
                N_ELEM, H, 0.0, BASE_SUPPORT)
    _add_column(model, 'Col2', 'E', 'C', 'Steel', COLUMN_SEC,
                N_ELEM, H, B1,  BASE_SUPPORT)
    if two_bay:
        _add_column(model, 'Col3', 'F', 'D', 'Steel', COLUMN_SEC,
                    N_ELEM, H, B1+B2, BASE_SUPPORT)

    # Beams
    _add_beam(model, 'Beam1', 'B', 'C', 'Steel', BEAM_SEC,
              N_ELEM, H, 0.0, B1)
    if two_bay:
        _add_beam(model, 'Beam2', 'C', 'D', 'Steel', BEAM_SEC,
                  N_ELEM, H, B1, B1+B2)
...
def _print_loads(two_bay):
    ...
    brace = ("Yes (DZ restrained at B, C" + (", D" if two_bay else "") + ")"
             if LATERAL_BRACE_BEAM_NODES else "No")
...
def main_column(p):
    ...
    print(f"  E*Iy = {EIy/1e3:.0f} kNm^2  (strong)    "
    ...
    print(SEP)
    print(f"  N_cr (weak,  Iz) = pi^2*{EIz/1e3:.0f}/{L_cr_th:.3f}^2"
          f" = {N_cr_z/1e3:.2f} kN   lam_cr = {N_cr_z/abs(P):.4f}")
    print(f"  N_cr (strong, Iy) = pi^2*{EIy/1e3:.0f}/{L_cr_th:.3f}^2"
    ...
    print(SEP)
    print(f"  {'Mode':>4}   {'lam_cr':>8}   {'N_cr [kN]':>10}  "
          f" {'L_cr,z [m]':>10}   {'K_z':>6}   "
          f"{'L_cr,y [m]':>10}   {'K_y':>6}")
    print(SEP)
    for i, lam in enumerate(lams):
        ...
        print(f"  {i+1:4d}   {lam:8.4f}   {N_cr/1e3:10.2f}"
    ...
    if len(lams) > 0:
        ...
        print(f"  FEM L_cr,z (mode 1) = {Lz0:.4f} m   theory = {L_cr_th:.4f} m"
              f"   error = {err:.2f}%")
    print()
    print("  Done.")
...
def _mcr_annex_f(E, Iz, G, J, Iw, L, C1, k, kw):
    ...
    kL  = k * L
    pi2 = np.pi**2
    return C1 * (pi2 * E * Iz / kL**2) * np.sqrt(
        (kw / k)**2 * Iw / Iz + kL**2 * G * J / (pi2 * E * Iz)
    )
...
def main_beam_mcr(p):
    ...
    print(f"    Sum (no Iw)                  = {term_sv:.6e} m^2   sqrt = {np.sqrt(term_sv):.5f} m")
    print(f"    Sum (with Iw)                = {term_warp+term_sv:.6e} m^2   sqrt = {np.sqrt(term_warp+term_sv):.5f} m")
    print()
    print(f"  M_cr (Iw = 0) = {C1*coeff/1e3:.4f} x {np.sqrt(term_sv):.5f} m = {abs(M_cr_noIw)/1e3:.4f} kNm")
    print(f"  M_cr (with Iw)= {C1*coeff/1e3:.4f} x {np.sqrt(term_warp+term_sv):.5f} m = {abs(M_cr_Iw)/1e3:.4f} kNm")
    print(SEP)
    print()
    ...
    print(SEP)
    print(f"  Without Iw:  {abs(M_max)/1e3:.4f} / {abs(M_cr_noIw)/1e3:.4f}"
          f" = {eta_noIw:.3f}  {pass_noIw}")
    print(f"  With Iw:     {abs(M_max)/1e3:.4f} / {abs(M_cr_Iw)/1e3:.4f}"
          f" = {eta_Iw:.3f}  {pass_Iw}")
    print(SEP)
    print()
...
def main():
    if SCENARIO in _COLUMN_PRESETS:
        main_column(_COLUMN_PRESETS[SCENARIO])
        return
    if SCENARIO in _BEAM_MCR_PRESETS:
        main_beam_mcr(_BEAM_MCR_PRESETS[SCENARIO])
        return
    ...
    print(SEP)
    print(f"  {'Member':8s}  {'Mode':>4}  {'Plane':>10}"
    ...
    for col in col_names:
        ...
        for mode_idx in range(n_modes):
            ...
            for plane_label, plane_char in (('y (strong)', 'y'),
                                            ('z (weak)',   'z')):
                try:
                    L_cr = results.effective_length(col, mode=mode_idx,
                                                    plane=plane_char)
                except Exception:
                    L_cr = float('nan')
                ...
                print(f"  {col:8s}  {mode_idx+1:4d}  {plane_label:>10}"
    ...
    for bname in beam_names:
        ...
        try:
            ...
            M_max = max(abs(bm.moment('Mz', x, combo_name=COMBO_NAME))
                        for x in x_pts)
